...
class DecisionTimeEvaluator:
    ...
    def evaluate(self):
        print("Starting game")
        main_board(3, white_type='Kane', black_type='Kane', callback=self.record_think_time)
    ...
    def plot_think_times(self):
        ...
        plt.plot(self.abel_times, marker='x', label='Kane (Black)')
        plt.title('Kane vs Kane - Decision Times')
        plt.xlabel('Move Number')
        plt.ylabel('Think Time (seconds)')
        plt.legend()
        plt.grid(True)
        plt.savefig('kane_vs_kane_decision_times.png')
        plt.show()
