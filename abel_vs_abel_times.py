...
class DecisionTimeEvaluator:
    ...
    def evaluate(self):
        print("Starting game")
        main_board(3, white_type='Abel', black_type='Abel', callback=self.record_think_time)
        self.plot_think_times()

    def plot_think_times(self):
        """Charting thinking time"""
        plt.figure(figsize=(10, 6))
        plt.plot(self.kane_times, marker='o', label='Abel (White)')
        plt.plot(self.abel_times, marker='x', label='Abel (Black)')
        plt.title('Abel vs Abel - Decision Times')
        plt.xlabel('Move Number')
        plt.ylabel('Think Time (seconds)')
        plt.legend()
        plt.grid(True)
        plt.savefig('abel_vs_abel_time.png')
        plt.show()
